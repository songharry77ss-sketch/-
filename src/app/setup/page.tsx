...
const RELATIONS = ["연인", "배우자", "친구", "가족", "동료"];

const SCENARIOS = [
  {
    label: "늦은 귀가",
    text: "어제 새벽 2시에 들어왔는데, 어디서 뭘 했는지 설명이 매번 다릅니다. 처음엔 친구라더니, 다음엔 일이라고 합니다.",
  },
  {
    label: "비밀 누설",
    text: "내가 저 사람한테만 말한 비밀을 다른 친구가 알고 있습니다. 추궁하니 '나는 말한 적 없다'고만 합니다.",
  },
  {
    label: "수상한 메시지",
    text: "휴대폰 알림창에서 모르는 사람이 보낸 다정한 메시지를 봤습니다. 물어보니 '잘못 온 거다'라고 합니다.",
  },
  {
    label: "사라진 물건",
    text: "내 서랍에서 현금이 없어졌고, 그 사람만이 들어올 수 있었습니다. 본인은 모른다고만 합니다.",
  },
];
...
export default function SetupPage() {
  ...
  const canStart = nameOk && situationOk;
  ...
  return (
    <main className="min-h-screen px-6 py-8 max-w-2xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-8 border-b border-ink-500 pb-3">
        <Link
          href="/"
          className="font-mono text-[11px] text-bone-300 hover:text-blood-300 tracking-widest transition-colors"
        >
          ◀ 처음으로
        </Link>
        <span className="font-mono text-[10px] text-bone-500 tracking-[0.3em]">
          {completed}/3 입력 완료
        </span>
      ...
      <p className="text-bone-300 text-sm mb-10">
        세 가지만 알려주시면, 프로파일러가 즉시 심문을 시작합니다.
      </p>
      ...
      <div className="space-y-7">
              ...
              <button
                ...
                className={`px-5 py-2.5 border text-sm transition-colors ${
                  relation === r
                    ...
                    : "border-ink-500 text-bone-300 hover:border-bone-300"
                }`}
              >
                {r}
              </button>
        ...
        <Field
          ...
          counter={`${situation.length}/1000`}
        >
          <textarea
            value={situation}
            onChange={(e) => setSituation(e.target.value)}
            placeholder="시간, 장소, 발견한 단서까지 적어주세요. '어제 늦게 들어왔는데, 처음엔 친구라더니...'"
            maxLength={1000}
            rows={5}
        ...
        >
          {canStart ? "▶ 심문실 입장" : `${3 - completed}개 더 입력하세요`}
        </button>

        <p className="text-[10px] font-mono text-bone-500 tracking-wider text-center leading-relaxed">
          입력하신 내용은 이 브라우저에만 저장되며 종료 시 삭제됩니다.
          <br />
          심문 보고서는 참고용이며 법적 효력이 없습니다.
        </p>
      </div>
    </main>
  );
  ...
  return (
    <div>
      <div className="flex items-baseline justify-between mb-2.5">
        <label className="flex items-baseline gap-3">
          <span
            className={`font-mono text-[11px] tracking-[0.3em] transition-colors ${
              done ? "text-emerald-400" : "text-blood-300"
            }`}
          >
            {done ? "✓" : "▶"} {step}
          </span>
        ...
        {counter && (
          <span className="font-mono text-[10px] text-bone-500">{counter}</span>
        )}
      </div>
      {children}
      {hint && (
        <p className="text-[10px] font-mono text-bone-500 mt-1.5 tracking-wider">
          {hint}
        </p>
      )}
    </div>
